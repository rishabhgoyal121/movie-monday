...
export default function TVShowSwitchDisplay({
  tvShowListType,
}: {
  ...
  useEffect(() => {
    const loadTVShows = async () => {
      ...
      const { data, error, message } = await fetchTVShows(tvShowListType, 1); // Await the async function

      if (data && data.results.length > 0) {
        setTVShows(data.results); // Ensure accessing `results` array
      }
  ...
  useEffect(() => {
    const ratingFilteredTVShows = tvShows?.filter((tvShow) => {
      return tvShow.vote_average >= filters.vote_average;
    });
    const fromReleaseDateFilteredTVShows =
      filters.release_date && filters.release_date.from
        ? ratingFilteredTVShows?.filter((tvShow) => {
            if (filters.release_date?.from) {
              const tvShowReleaseDate = new Date(tvShow.first_air_date);
              return tvShowReleaseDate >= filters.release_date?.from;
            }
            return true;
          })
        : ratingFilteredTVShows;
    const toReleaseDateFilteredTVShows =
      filters.release_date && filters.release_date.to
        ? fromReleaseDateFilteredTVShows?.filter((tvShow) => {
            if (filters.release_date?.to) {
              const tvShowReleaseDate = new Date(tvShow.first_air_date);
              return tvShowReleaseDate <= filters.release_date?.to;
            }
            return true;
          })
        : fromReleaseDateFilteredTVShows;
    setFilteredTVShows(toReleaseDateFilteredTVShows);
  }, [filters, tvShows]);
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 p-4">
                  {filteredTVShows &&
                    ...
                    filteredTVShows?.map((tvShow) => {
                      return (
                        <Card key={tvShow.id} className="w-full bg-gray-800 border-gray-700 hover:border-blue-500 transition-all duration-300">
